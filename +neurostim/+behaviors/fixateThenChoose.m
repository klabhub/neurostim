classdef fixateThenChoose < neurostim.behaviors.fixate
    ...
    methods
        ...
        function fixating(o,t,e)
            ...
            if isAllowedBlink
                % Stay in state.
            elseif (~fixDone && ~inFix) || (choiceShouldHaveStarted && ~inChoice)
        ...
        function choose(o,t,e)
            ...
            % Guards
            [inChoice,isAllowedBlink] = isInWindow(o,e,o.choice); % Check that we're still in the window around the original choice
            choiceComplete = o.duration >= o.choiceDuration;
            isCorrect = o.correct;
            % All transitions
            if isAllowedBlink
                % No change in state
            elseif choiceComplete || e.isAfterTrial
    ...
    methods
        ...
        function [v,targetIx,isAllowedBlink] = isInAnnulus(o,e)
            targetIx = [];
            if ~e.valid

 ...
 * which have a main window for (monochrome) high bit depth images and
 * an overlay window with indices into a color look up table.
 * The devices then duplicate the output and can apply different look up tables
 * which allows for a somewhat convenient way to add experimenter information to
 * a mirrored display.
 * 
 * This shader expects that the window is twice the size of the display screen and 
 * covers the display screen and a screen for an experimenter copy.
 * 
 ...
 *
 * DIFFERENCE TO HARDWARE IMPLEMENTATIONS: Color is preserved as there is no benefit 
 * in monochrome output.
 *
 * Non-zero indices drawn into an overlay pointer will replace the value from the main pointer
